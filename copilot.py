...
def decompress_image(image_path,index):
    default_path = "\\tmp\\" 
    root_path = os.path.dirname(os.path.abspath(__file__))
    tmp_path = f"{root_path}{default_path}"
    original_image = Image.open(image_path)
    resize_image = original_image.resize((400,400), Image.LANCZOS)
    if original_image.height > original_image.width :
        resize_image = original_image.resize((400,800), Image.LANCZOS)
    elif original_image.height < original_image.width:
        resize_image = original_image.resize((800,400), Image.LANCZOS)
    elif original_image.height == original_image.width:
        resize_image = original_image.resize((800,800), Image.LANCZOS)
    resize_image.save(f"{tmp_path}images{index}.jpg", optimize=True, quality=100)
    return f"{tmp_path}images{index}.jpg"

async def main(image_path,index) -> None:
    async with SydneyClient() as sydney:
        tmp = decompress_image(image_path,index)
        response = await sydney.ask("give json , with title without special char , and description and tags (array string) minimum 15 and more is better. only title,description,tags", attachment=tmp)
